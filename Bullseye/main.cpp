...
char TARGETNAME[64] = "Untitled - Notepad"; //Target WindowName Here!
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	WINNAME = random_string(20); //epic eac process walker baipass
	...
	UnregisterClass(WINNAME.c_str(), (HINSTANCE)hInst);
	FreeLibraryAndExitThread((HMODULE)hInst, 0);
	return 0;
}

HWND WINAPI InitializeWin(HINSTANCE hInst) {
	std::string randomName = random_string(6);
	...
	wndClass.lpszClassName = WINNAME.c_str();
	wndClass.lpszMenuName = WINNAME.c_str();
	...
	Globals::hWnd = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED, WINNAME.c_str(), WINNAME.c_str(), WS_POPUP, 1, 1, Globals::rWidth, Globals::rHeight, 0, 0, 0, 0);
